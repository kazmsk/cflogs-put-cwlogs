...
exports.handler = (event, context, callback) => {
  ...
  co(function* () {
    ...
    // unzip cloudfront log
    const unzipdata = yield gunzip(data);
    const messages = unzipdata.toString();

    // create logdata
    console.log('create logdata');
    const logEvents = [];
    messages.split('\n').forEach((value, index) => {
      if (value.length > 0 && index >= 2) {
        const timestamp = moment(value.split(/\s+/)[0] + ' ' + value.split(/\s+/)[1]).valueOf();
        const logEvent = {
          message: value,
          timestamp: timestamp
        };
  ...
  function gunzip(data) {
    return new Promise((resolve, reject) => {
      zlib.gunzip(data.Body, (error, buffer) => {
        if (!error) {
          resolve(buffer);
        } else {
          reject(error);
